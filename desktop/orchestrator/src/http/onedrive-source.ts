...
import { MAX_DOCUMENT_BYTES } from './attachments.ts';
import { convertDocumentToMarkdown } from './document-conversion.ts';
import {
  asString,
...
export function isOneDriveIngestionEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.VERSO_ONEDRIVE_INGESTION_ENABLED?.trim().toLowerCase() === 'true';
}
